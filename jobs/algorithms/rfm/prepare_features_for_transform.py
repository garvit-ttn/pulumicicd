...
def rfm_transform(trx: DataFrame, prod: DataFrame) -> DataFrame:
...
if __name__ == '__main__':
    ...
    result = rfm_transform(trx_df, prod_df)

    path_to_files = get_s3_path_for_feature(features.RfmTransformInput, lake_descriptor, data_filter_context)

    if result.count() < 100000:
        result.write.mode("overwrite").option("header", "false").csv(path=path_to_files)
    else:
        result.repartition(int(result.count() / 100000)).write.mode("overwrite").option("header", "false").csv(path=path_to_files)
