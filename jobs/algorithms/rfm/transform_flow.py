...
def generate_workflow_definition(
    ...
    pass_crawler = Pass(state_id=f"Pass-rfm-crawler_name")
    ...
    rfm_job_name = "rfm-inference"

    rfm_inference_job = transform_step(rfm_job_name, retry)
    ...
    start_crawler = Task(
        state_id=f"Start-rfm-crawler_name",
    ...
    flow = Workflow(
        name="cvm-rfm-transform-workflow",
...
def transform_step(rfm_job_name, retry):
    return Task(
        state_id=f"Run-{rfm_job_name}"[0:80],
        input_path="$",
        result_path="$.result",
        output_path="$",
