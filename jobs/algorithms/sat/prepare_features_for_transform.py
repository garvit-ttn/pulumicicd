...
def filter_for_bu_context(trx: DataFrame, prod: DataFrame, demo: DataFrame, pao: DataFrame, idmap: DataFrame,
    ...
    demo = (
        ...
        .join(
            how='inner', other=idmap.alias(conformed.IdMapping.table_alias()),
            on=col(
                conformed.Demographics.idi_counterparty.column_alias()) == col(
                conformed.IdMapping.idi_src.column_alias()))

    )
    demo = (
        # demo.filter(col(demo_filter_column.name) == demo_filter_value)
        demo.drop(col(conformed.Demographics.idi_counterparty.name))
        .withColumn(conformed.Demographics.idi_counterparty.name,
                    col(conformed.IdMapping.idi_gcr.name).cast(StringType()))
    )
    print(demo.show())
    ...
    pao = pao.filter(col(conformed.ProfileAlgorithmsOutput.data_filter_context.name) == "rfm.bu.share.brand5")  # TODO
...
def sat_transform(trx: DataFrame, trx_cat: DataFrame, prod: DataFrame, demo: DataFrame, pao: DataFrame) -> DataFrame:
    ...
    # joined_trx_cat = joined_trx_cat.filter(col(conformed.Demographics.adr_visit_cde_country.name) == 'AE')

    joined_trx_cat = joined_trx_cat.select("idi_counterparty").dropDuplicates()

    print("10_joined_trx_prod only idi_counterparty")
    joined_trx_cat.show()
...
if __name__ == '__main__':
    ...
    result = sat_transform(trx_df, trx_cat_df, prod_df, demo_df, pao_df)

    path_to_files = get_s3_path_for_feature(features.SatTransformInput, lake_descriptor, data_filter_context)

    result.write.mode("overwrite").option("header", "false").csv(
        path=path_to_files)
