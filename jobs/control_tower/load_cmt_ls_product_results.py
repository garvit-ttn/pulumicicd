...
# result = df.filter((col("bu") == "fsn") &
#                    (col("dat_date_type_1").between("2021-05-01", "2023-04-30")) &
#                    (col("bu") == "share")
#                    ).select(
#                         countDistinct("txn_id").alias("total_txns"),
#                         sum("cua_amount_type_2").alias("gross_revenue"),
#                         sum("cua_amount_type_1").alias("net_revenue"),
#                         sum(when(col("des_order_status") == "cancelled", col("cua_amount_type_2"))).alias("return_sales"),
#                         countDistinct(when(col("des_order_status") == "cancelled", col("txn_id"))).alias("return_txns"),
#                         (sum("cua_amount_type_2") / countDistinct("txn_id")).alias("atv"),
#                         (countDistinct("item_no") / countDistinct("txn_id")).alias("abs"),
#                         (sum("cua_amount_type_2") / countDistinct("item_no")).alias("asp"),
#                         countDistinct("item_no").alias("quantity"),
#                         sum("Net_sales_discount").alias("total_discount")
#                    )
