...
def run(audiences: DataFrame, logger, braze_sync_api_key: str, braze_sync_auth_url: str,
    ...
    if conformed.Audiences.campaign_end_date.name in audiences.columns:
        audiences = audiences.filter(col(conformed.Audiences.campaign_end_date.name)
                                     .isNotNull()).filter((col(conformed.Audiences.campaign_end_date.name)
                                                           >= current_date()))

    audiences = audiences.filter(col("active_appgroup") == True).orderBy(desc(col('ingestion_timestamp')))
    eligible_campaigns = audiences.select('audience').distinct()
    print("Eligible Campiagns to be sent to Braze", eligible_campaigns.show(truncate=False))

    bu_info = audiences.filter(col("active_appgroup") == True).select("business_unit").distinct()
    bu_info.show()

    bu_info = bu_info.collect()
    ...
    print("Total Requests sent to Braze : ", audiences.count())
    ...
    for bu in bu_info:
        audiences_local = audiences.filter(col("business_unit") == bu['business_unit'])
        audiences_local.show()

        audiences_local = audiences_local.coalesce(1)
        audiences_local.show()
        result_local = audiences_local.rdd.mapPartitions(send_to_braze_over_partitions)

        if not result_local.isEmpty():
            result_local = result_local.toDF(['payload', 'result'])
            results = (
                results.union(result_local)
                if results is not None else result_local
            )
        else:
            print(f"result is empty for {bu['business_unit']}")

        failed = results.filter(col('result') != '200')
        success = results.filter(col('result') == '200')

        logger.info(f"Total requests for:{bu['business_unit']} {results.count()}")
        logger.info(f"Requests failed :{bu['business_unit']} {failed.count()}")
        logger.info(f"Requests success:{bu['business_unit']} {success.count()}")
        failed.show(50, truncate=False)

    if results is None:
        raise Exception("There is no results were pushed!")
...
if __name__ == '__main__':
    ...
    audience_df: DataFrame = create_delta_table_from_catalog(spark, conformed.AudiencesBraze, lake_descriptor).toDF()
    audience_df.show(1)
    result = run(
