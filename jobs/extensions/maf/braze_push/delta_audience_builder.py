...
def overwrite_nbo_with_static_offers_for_audience(
    ...
    if profile_measurements is None:
        ...
        audience11 = audience.join(profile_measurements,
                                                   audience.idi_counterparty == profile_measurements.idi_counterparty,
                                                   "left_anti")

        # added as a request for a new table to fetch ugc records been excluded
        ugc_audience = audience.join(profile_measurements,audience.idi_counterparty == profile_measurements.idi_counterparty,"inner")
        ugc_audience = ugc_audience.select(audience.idi_counterparty
                                           , audience.audience, profile_measurements.idi_measurement)
        ugc_audience = ugc_audience.withColumnRenamed('idi_counterparty','gcr_id')
    print(f"Total gcr_id's after UCG group exclusion {audience11.count()}")
    ...
    if excluded_aud is None:
        ...
        audience11 = audience11.join(excluded_aud, audience11.idi_counterparty == excluded_aud.idi_counterparty,
                                 "left_anti")

    print(f"Audience after mapping to excluded audience : {audience11.count()}")

    # Enrich audiences with braze id
    audience_with_ids = enrich_with_customer_src_id(
        input_df=audience11, id_map=id_map,
    ...
    generic_offer_mapping = enrich_generic_offer_mapping(audience_with_ids, generic_promo_ids)
    targetted_offer_mapping = enrich_targetted_offer_mapping(audience_with_ids, share_audience, targetted_promos)
    ...
    return offer_mapping, static_offers,ugc_audience
...
def final_prep_for_nbo_to_overwrite(nbo_to_overwrite: DataFrame) -> tuple[DataFrame, DataFrame]:
    ...
    share_to_upsert.show(5,truncate=False)
    braze_to_upsert.show(5,truncate=False)
...
def braze_audience_builder(
    ...
    ugc_audiences: Optional[DataFrame] = None
    ...
    for ai in aud_info:
        ...
        if audience_count == 0:
            ...
            mapped_to_braze_id, static_offers_for_audience,ugc_audience = overwrite_nbo_with_static_offers_for_audience(
                audience=audience,
                aud_info=ai,
                promos=promos,
        ...
        ugc_audiences = ugc_audience if ugc_audiences is None else ugc_audiences.union(ugc_audience)
    ...
    if audiences.count() == 0:
        raise Exception("There are zero audiences to push")

    audiences.show(5, truncate=False)
    # processed_audiences.show(5, truncate=False)
    # filtered_audiences1.show(5, truncate=False)
    # ugc_audiences.show(2, truncate=False)
...
def audience_push_to_braze(spark_session: SparkSession, audiences: DataFrame, audiencesbraze_df: DataFrame,
    ...
    audiencesbraze_df1 = audiencesbraze_df.filter((col(conformed.Audiences.campaign_end_date.name)
                                                           >= current_date()))

    bu_join: DataFrame = (
        audiencesbraze_df1.join(bu_info, audiencesbraze_df1.business_unit == bu_info.business_unit,
                                "inner")).withColumn(
    ...
    main_df = bu_join
    main_df = main_df.dropDuplicates(['external_id', 'gcr_id', 'share_id', 'audience', 'business_unit','campaign_end_date'])
    ...
    for c in campaign_distinct:
        ...
        if audience_count == 0:
            ...
            #                                           "left_anti")
            delta_audience_ids = per_audience_df
...
if __name__ == '__main__':
    ...
    # write_delta(
    #     result.ugc_audience, conformed.AudiencesBrazeUCG, spark, lake_descriptor, save_mode="append"
    # )

    creat_delta_table_if_not_exists(spark, conformed.AudiencesBrazeUCG, lake_descriptor)
    audience_path1 = get_s3_path(conformed.AudiencesBrazeUCG, lake_descriptor)
    result.ugc_audience.write.format('delta').mode('append').save(audience_path1)
