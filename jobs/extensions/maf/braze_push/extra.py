...
from pyspark.sql.functions import (
    ...
    current_timestamp, current_date, format_string, explode, split, regexp_extract,
    concat, when, collect_set
)
...
def overwrite_nbo_with_static_offers_for_audience(
        ...
        excluded_aud: DataFrame, profile_measurements: DataFrame
    ...
    #     audience = audience.join(profile_measurements,
    #                              audience.idi_counterparty == profile_measurements.idi_counterparty, "left_anti")
    # print(f"Total gcr_id's after UCG group exclusion {audience.count()}")
...
def exclude_audience(spark_session: SparkSession, profiles1: DataFrame,
    ...
    ).withColumn("exclusion_id", explode(split(col("exclusion_audiences_concat"), "\+"))).withColumnRenamed('status','campaign_status')
    ...
    for ai in exclude_aud_info:
        ...
        #     '20230821_UAE_XBU_XBU UCG Campaign not to braze_1_braze_358',
        # ]
...
def braze_audience_builder(
        ...
        excluded_audiences: DataFrame, profile_measurements: DataFrame
    ...
    campaigns = campaigns.withColumn(
        landing.Campaign.audience_id.name,
        explode(split(landing.Campaign.audiences_concat.name, "\+"))
    ).withColumnRenamed('status','campaign_status')
    ...
    print("profile_measurements", profile_measurements.show())
    ...
    for ai in aud_info:
        ...
        if audience_count == 0:
            ...
            if excluded_audiences is None:
                ...
                excluded_audience_count = excluded_audiences.filter(col('audience') == ai[landing.Campaign.name.name]).count()
    ...
    audiences = audiences.withColumnRenamed('braze_id','external_id')
...
if __name__ == '__main__':
    ...
    braze_attributes = spark.read.parquet('s3://cvm-prod-landing-5d6c06b/bu_external_id/2023-08-22/')
    ...
    result = braze_audience_builder(
        ...
        offers_bank_offers, delta_df, excluded_audiences, profile_measurements
    )
