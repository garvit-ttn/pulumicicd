...
class CvmConformedZone:
    def __init__(self, crawler_role: Input[Role], prefix: str, bucket_name: Input[str] = None):
        ...
        self.conformed_bucket = self._cvm_conformed_data_bucket()
        self.conformed_db = self._cvm_conformed_db()

        self.lake_descriptor: dict[str, Input[str]] = {}
        self.crawlers_by_spec: dict[str, Input[str]] = {}
        self.tables_by_spec: dict[str, Input[str]] = {}

    def _cvm_conformed_db(self):
        return aws.glue.CatalogDatabase(f"cvm_{self.prefix}_conformed_db", aws.glue.CatalogDatabaseArgs(
            name=f"cvm_{self.prefix}_conformed_db"  # Valid names only contain alphabet characters, numbers and _
        ))
    ...
    def _cvm_conformed_data_bucket(self):
        area = "conformed"
        ...
        bucket = aws.s3.Bucket(bucket_name, aws.s3.BucketArgs(
            ...
            tags=get_tags(Owner.Exsell, Stage.Enrichment, bucket_name)
    ...
    @property
    def conformed_bucket_id(self) -> Output[str]:
        if isinstance(self.conformed_bucket, AwaitableGetBucketResult):
            return Output.from_input(self.conformed_bucket.id)

        if isinstance(self.conformed_bucket, Bucket):
            return self.conformed_bucket.id

    def cvm_conformed_table_from_spec(self, table: Type[TableSpec]):
        partitions = []
        columns = []
        ...
        tbl, crawler = self.cvm_conformed_table(
        ...
        self.tables_by_spec[table.table_id()] = self.conformed_bucket_id.apply(
            lambda x: f"s3://{x}/{table.table_name()}"
        )

        self.crawlers_by_spec[table.table_id()] = crawler.name

        self.lake_descriptor[table.table_id()] = self.conformed_bucket_id.apply(
            lambda x: f"s3://{x}/{table.table_name()}"
        )

    def cvm_conformed_table(
        ...
        full_table_name = f"cvm_{self.prefix}_conformed_{tbl_name}"
        tbl_location = self.conformed_bucket_id.apply(lambda x: f"s3://{x}/{tbl_name}/")

        table = aws.glue.CatalogTable(f"{full_table_name}_table", aws.glue.CatalogTableArgs(
            database_name=self.conformed_db.name,
            name=full_table_name,
            description=description,
            storage_descriptor=aws.glue.CatalogTableStorageDescriptorArgs(
                columns=columns,
                location=self.conformed_bucket_id.apply(lambda x: f"s3://{x}/{tbl_name}/_symlink_format_manifest"),
        ...
        crawler = aws.glue.Crawler(crawler_name, aws.glue.CrawlerArgs(
            ...
            #                   }
            #                 }""",
            tags=get_tags(Owner.Exsell, Stage.Enrichment, crawler_name)
        ), opts=ResourceOptions(depends_on=[self.conformed_db, table]))
